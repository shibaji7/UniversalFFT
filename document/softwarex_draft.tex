...
scaling, for example, numpy divides the inverse transform (\texttt{ifft()}) by $N$; 
R's \texttt{fft(inverse=TRUE)} does not; IDL's \texttt{FFT()} divides the \emph{forward} 
transform by $N$. For a filter applied as a forward--inverse pair these factors cancel, and
...
mis-scaled by a factor of $N\Delta t$ or $N\Delta f$, which leads to incorrect physical interpretations.
...
All code is made open-source under the MIT licence with 100\% test coverage.
\end{abstract}
...
direction carries a $\frac{1}{N}$ factor (or $\Delta t$, or $\Delta f$), and which sign
appears in the complex exponential. Any specific library implementation commits
to one choice from this family. While the choice is made open by the library's documentation, 
but if the user does not read the documentation carefully, they may inadvertently use the 
wrong convention, that leads to incorrect physical interpretations.
...
single transforms, if the user is interfacing with the different tools/libraries/languages 
for a round-trip calculation. The inverse DFT of a brick-wall low-pass transfer function
should recover the sinc impulse response, whose integral over time equals $1$
--- a consequence of the filter passing DC with unit gain. With Python's NumPy convention
this integral is approximately $\frac{1}{\Delta t}$; with R's it is $\frac{N}{\Delta t}$. Neither
...
\midrule
NumPy \texttt{fft} / \texttt{ifft}              & $1$     & $\frac{1}{N}$ \\
MATLAB \texttt{fft} / \texttt{ifft}             & $1$     & $\frac{1}{N}$ \\
R \texttt{stats::fft(inverse=TRUE)}             & $1$     & $1$ (raw sum) \\
FFTW \texttt{FFTW\_FORWARD / BACKWARD}          & $1$     & $1$ \\
IDL \texttt{FFT(x,-1)} / \texttt{FFT(X,+1)}    & $\frac{1}{N}$   & $1$ \\
\bottomrule
\end{tabular}
\end{table}
...
introduce a $2\pi$ factor in Parseval's theorem), study derived two distinct DFT
...
\begin{itemize}
  \item \textbf{CC pair (Boteler Eqs.\ 21a/21b)} --- both the time and frequency
    ...
    $\Delta f = \frac{1}{N\Delta t}$. This pair is the correct choice when recovering
...
Boteler~(2012) is a \emph{theoritical} study. It stops at the equations and explicitly
...
environments --- Python, MATLAB, Fortran, Julia, R, and many others --- have been left
...
$X[k]$ their spectrum, and $\Delta f = \frac{1}{N\Delta t}$ the frequency resolution.
...
The roundtrip identity $\Delta t \cdot \Delta f = \frac{1}{N}$ guarantees that applying
...
Filter (FFT $\to$ multiply $\to$ IFFT) & Either &
  $\Delta t \cdot \Delta f = \frac{1}{N}$ cancels across the pair \\
...
Discrete spectrum of periodic waveform & CD forward &
  Fourier-series coefficients; spike amplitude $= \frac{A}{2}$ \\
\bottomrule
\end{tabular}
\end{table}
...
only the import statement; the transform calls remain unchanged. Addiionally, for cases where user needs
to switch between different languages while computing, the same API can be used and without worrying about 
...
C / C\texttt{++} / Fortran / Rust / JS / CUDA & raw (self-contained) & $\times\Delta t$ & $\frac{1}{N}$ \\
R \texttt{fft(inverse=TRUE)}                & raw (no $\frac{1}{N}$) & $\times\Delta f$ direct & identity \\
IDL \texttt{FFT(x,-1)}                      & $\frac{1}{N}$ (IDL convention) & $\times N\Delta t$ & identity \\
\bottomrule
\end{tabular}
\end{table}
